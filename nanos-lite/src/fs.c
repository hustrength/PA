...
size_t fs_read(int fd, void *buf, size_t len) {
    ...
    if (file_table[fd].read == NULL)
        length = ramdisk_read(buf, \
        file_table[fd].disk_offset + file_table[fd].open_offset, r_len);
    else
        length = file_table[fd].read(buf, \
        file_table[fd].disk_offset + file_table[fd].open_offset, r_len);
...
size_t fs_write(int fd, const void *buf, size_t len) {
    ...
    if (file_table[fd].write == NULL)
        length = ramdisk_write(buf, \
        file_table[fd].disk_offset + file_table[fd].open_offset, w_len);
    else
        length = file_table[fd].write(buf, \
        file_table[fd].disk_offset + file_table[fd].open_offset, w_len);
